      ...
      .filter(purchase => {
        if (filters.search) {
          ...
          return purchase.invoice_number.toLowerCase().includes(searchTerm) ||
                 String(purchase.supplier_id).includes(searchTerm);
        }
        if (filters.date_range) {
          const dateRange = filters.date_range as { start: string; end: string };
          if (dateRange.start && dateRange.end) {
            const purchaseDate = new Date(purchase.invoice_date).getTime();
                  ...
                  <TableRow key={purchase.purchase_id}>
                    ...
                    <TableCell>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <IconButton
                          size="small"
                          onClick={() => navigate(`/transactions/purchase/${purchase.purchase_id}/edit`)}
                          title="Edit"
                        >
                          <Edit />
                        </IconButton>
